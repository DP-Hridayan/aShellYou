...
class CreateQsTileTool @Inject constructor(
    ...
    override suspend fun execute(args: KJsonObject?): String {
        ...
        val rawMode = args["execution_mode"]?.jsonPrimitive?.content?.toIntOrNull() ?: TileExecutionMode.SHIZUKU
        ...
        val isToggleable = args["is_toggleable"]?.jsonPrimitive?.content?.toBooleanStrictOrNull() ?: false
        ...
        val tileId = if (requestedSlot != null && requestedSlot in 1..10 && existing.none { it.id == requestedSlot }) {
            requestedSlot
        } else {
        ...
        val typeStr = if (isToggleable) "Toggleable (ON: '$command', OFF: '$inactiveCmd')" else "Tap Action ('$command')"
