...
import `in`.hridayan.settingsgraph.graph.RadioGroupItemBuilder
import `in`.hridayan.settingsgraph.graph.SettingsGraphMarker
import `in`.hridayan.settingsgraph.graph.SwitchItemBuilder
import `in`.hridayan.settingsgraph.model.ItemBehavior

/**
 * Holds global default callbacks used by [SettingsColumn] when no
 * per-item override is defined in the graph block.
 *
...
/**
 * graph scope for building [OnClickDefaults].
 *
 * Use this inside [rememberSettingsGraphState] to register global default callbacks.
 */
@SettingsGraphMarker
...
/**
 * Builds an [OnClickDefaults] instance using the [OnClickDefaultsScope] graph.
 *
