...
/**
 * Shown when an AI feature is used while the chosen provider has no API key.
 *
 * Distinct from [ApiKeyRequiredDialog]: here the user already has a provider selected and possibly
 * a key for a different one, so the useful action is switching providers rather than navigating off
 * to add a key.
 */
@Composable
fun NoKeyForActiveProviderDialog(
    provider: LlmProvider,
    onDismiss: () -> Unit,
    onChooseProvider: () -> Unit,
) {
    DialogContainer(onDismiss = onDismiss) {
        ...
        Text(
            modifier = Modifier.fillMaxWidth(),
            text = stringResource(
                R.string.no_api_key_for_active_provider,
        ...
        OverflowButtonGroup(
            items = listOf(
                ...
                ButtonGroupItem(
                    buttonConfig = ButtonConfigDefaults.defaultConfig(type = ButtonType.Button),
