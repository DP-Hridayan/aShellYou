package `in`.hridayan.composecolorkit.scheme
...
import `in`.hridayan.composecolorkit.palette.a1
import `in`.hridayan.composecolorkit.palette.a2
import `in`.hridayan.composecolorkit.palette.a3
import `in`.hridayan.composecolorkit.palette.n1
import `in`.hridayan.composecolorkit.palette.n2
import `in`.hridayan.composecolorkit.provider.SeedColorProvider
import `in`.hridayan.composecolorkit.scheme.ComposeColorKit.harmonizeWithPrimary
import `in`.hridayan.composecolorkit.seed.SeedColors
import `in`.hridayan.composecolorkit.seed.SeedDefaults

object ComposeColorKit {
    ...
    fun buildColorScheme(
        ...
        highContrastDarkMode: Boolean = false,
        expressiveLightColorScheme: Boolean = false,
        seedColors: SeedColors = SeedDefaults.seedColors(),
    ): ColorScheme {
        setSeedColor(seedColors)

        val colorScheme = when {
            dynamicColor && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> {
                if (darkMode && highContrastDarkMode) highContrastDynamicDarkColorScheme(context)
                else if (darkMode) dynamicDarkColorScheme(context)
                else dynamicLightColorScheme(context)
            }

            darkMode -> {
                if (highContrastDarkMode) highContrastDarkColorSchemeFromSeed()
                else darkColorSchemeFromSeed()
            }

            else -> lightColorSchemeFromSeed(expressiveLightColorScheme)
        }

        return colorScheme
    }

    fun setSeedColor(seedColors: SeedColors) {
        SeedColorProvider.setSeedColor(seedColors)
    }
...
@ExperimentalMaterial3ExpressiveApi
@Composable
private fun lightColorSchemeFromSeed(expressiveLightColorScheme: Boolean): ColorScheme {
    return if (expressiveLightColorScheme) expressiveLightColorScheme().copy(
        primary = 40.a1,
...
@Composable
private fun darkColorSchemeFromSeed(): ColorScheme {
...
@Composable
private fun highContrastDarkColorSchemeFromSeed(): ColorScheme {
...
private fun highContrastDynamicDarkColorScheme(context: Context): ColorScheme {
