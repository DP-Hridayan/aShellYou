...
public class WifiAdbShell {
    ...
    public static void execCommand(Context context, Activity activity) {
        ...
        try {
            ...
            while ((line = mError.readLine()) != null) mOutput.add("<font color=#FF0000>" + line + "</font>");
    ...
    public static boolean isBusy() {
        return mOutput != null && mOutput.size() > 0 && !mOutput.get(mOutput.size() - 1).equals(Utils.shellDeadError());
    }
    ...
    private static boolean doesAdbCommandNeedStorageAccess(String command) {
        String[] storageCommands = {
                "push", "pull", "backup", "restore", "install", "uninstall",
                "cp", "mv", "rm", "rmdir", "mkdir", "ls", "stat",
                "find", "touch", "chmod", "chown"
        };
