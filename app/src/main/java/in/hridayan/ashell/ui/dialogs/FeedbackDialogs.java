...
public class FeedbackDialogs {
  ...
  public static void outputSavedDialog(Context context, boolean saved) {
    ...
    if (saved) {
      builder.setPositiveButton(
          ...
          (dialog, i) -> Utils.openTextFileWithIntent(Preferences.getLastSavedFileName(), context));
    }
  ...
  public static void githubDownloadWarning(
      Context context,
      Activity activity,
    ...
    new MaterialAlertDialogBuilder(context)
        ...
        .setMessage(context.getString(R.string.github_download_warning))
        .setPositiveButton(
            context.getString(R.string.ok),
            (dialog, which) -> {
              // Start the update download
              AppUpdater.fetchLatestReleaseAndInstall(
                  activity, progressBar, descriptionTextView, loadingDots, downloadButton);
            })
        .setNegativeButton(
